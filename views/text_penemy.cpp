...
void PEnemyViewText::handlePoisonUpdated(int range, float poisonLevel) {
    ...
    for (int k = 0; k < 4; k++) {
        QPixmap *pTempPix = new QPixmap((xEnd - xStart + 1) * height,
                                        (yEnd - yStart + 1) * height); // Set width and height as needed
        ...
        for (int i = xStart; i <= xEnd; i++) {
            for (int j = yStart; j <= yEnd; j++) {
                ...
                // }
                painter.drawPixmap((i - xStart) * height, (j - yStart) * height, widthToCopy, widthToCopy,
                                   viewPoison->copy(k * widthToCopy, 0, height, height));
            }
        }
