...
template<typename T>
concept validNodeType = validPosType<T> && requires(T
t)
{
{
T::f} ->
...
std::vector<int>
A_star(std::vector <T> &Nodes, const U *start, const U *destination, Comparator<T> &comp, unsigned int width,
       float heurWeight) {
        ...
        for (int deltaY = -1; deltaY <= 1; deltaY++)
            for (int deltaX = -1; deltaX <= 1; deltaX++) {
                ...
                if (!(deltaX == 0 && deltaY == 0)) {
                    ...
                    checkNeighbour(Nodes, width, currentX + deltaX, currentY + deltaY, destination, parent, openList,
                                   heurWeight);
                }
            }
        if (openList.empty()) {
            ...
            qCDebug(pathfinderCategory) << "currentPathNode from openlist: " << currentPathNode.getXPos() << ", "
                                        << currentPathNode.getYPos() << ", f: " << currentPathNode.h;
    ...
    while (dest->prev != nullptr) {
        ...
        switch (newMove) {
            ...
            case -9:
                ...
                break;
            default:
                std::cerr << "wrong value for deltaX" << deltaX;
        }
        dest = dest->prev;
    }
...
void checkNeighbour(std::vector <T> &Nodes, int width, int nX, int nY, const U *destination, T *parent, pq<T> &openList,
                    float heurWeight) {
    QLoggingCategory pathfinderCategory("pathfinder", QtDebugMsg);
    ...
    if (nX >= 0 && nX < width && nY >= 0 && nY < height && (nX != 0 || nY != 0)) {
        ...
        {
            int manhDist = abs(static_cast<int>(destination->getXPos()) - nX) +
                           abs(static_cast<int>(destination->getYPos()) - nY);
            float f = (std::fabs(Nodes.at(currentY * width + currentX).getValue() -
                                 Nodes.at(nY * width + nX).getValue())) + 1.0f;
            qCDebug(pathfinderCategory) << "Manhattan distance from "
                                        << Nodes.at(currentY * width + currentX).getValue() << "x"
                                        << Nodes.at(nY * width + nX).getValue() << ";" << currentX << "x" << currentY
                                        << ";"
                                        << destination->getXPos() << "," << destination->getYPos() << " to " << nX
                                        << "," << nY << " is " << manhDist
                                        << ", f is " << f;
            ...
            if (nextNode.visited == false) {
                ...
                qCDebug(pathfinderCategory) << "Pushing in openlist... " << nX << ", " << nY << " currentCost:"
                                            << nextNode.h;
                openList.push(nextNode);
            } else {
                //check if current path is better than previous
                qCDebug(pathfinderCategory) << "Check if this path is better, currentCost:" << currentCost << ", f:"
                                            << f << ", nextnode.f:" << nextNode.f;
