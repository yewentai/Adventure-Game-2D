            ...
            for (int deltaY = -1; deltaY <= 1; deltaY++)
                for (int deltaX = -1; deltaX <= 1; deltaX++) {
                    ...
                    if (!(deltaX == 0 && deltaY == 0)) {
                        ...
                        checkNeighbour(Nodes, width, currentX + deltaX, currentY + deltaY, destination, parent,
                                       openList, heurWeight);
                    }
                }
            if (openList.empty()) {
...
private:
    void
    checkNeighbour(std::vector <T> &Nodes, int width, int nX, int nY, const U *destination, T *parent, pq<T> &openList,
                   float heurWeight) {
        ...
        if (nX >= 0 && nX < width && nY >= 0 && nY < height && (nX != 0 || nY != 0)) {
            if (Nodes.at(nY * width + nX).getValue() != std::numeric_limits<float>::infinity()) //top
            {
                ...
                qCDebug(pathfinderCategory) << "Manhattan distance from "
                                            ...
                                            << Nodes.at(nY * width + nX).getValue() << ";" << currentX << "x"
                                            << currentY << ";"
