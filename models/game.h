...
class GameModel {
public:
    GameModel(QString fileName, unsigned int nrOfEnemies, unsigned int nrOfHealthpacks, float pRatio = 0.25f,
              float hp = 100.0f);
    ...
    iterator findTile(int x, int y);

    std::vector<std::unique_ptr < Enemy>>::

    iterator findEnemy(int x, int y);

    std::vector<std::unique_ptr < HealthPack>>::

    iterator findHealthPack(int x, int y);

    std::vector<std::unique_ptr < Gate>>::

    iterator findGate(int x, int y);
